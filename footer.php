<footer class="footer">
  <div class="container">
    <div class="footer-body">
      <a href="tel:+88005507940" class="footer-tell--mob">8 800 550 79 40</a>
      <div class="footer-tell__text--mob">Звонок бесплатный</div>
      <div class="footer-item footer-contacts">
        <a href="tel:+88005507940" class="footer-tell">8 800 550 79 40</a>
        <div class="footer-tell__text">Звонок бесплатный</div>
        <a href="/terms-of-use.php" class="footer-privacy">Пользовательское соглашение</a>
        <a href="/privacy-policy.php" class="footer-privacy">Политика конфиденциальности</a>
        <ul class="footer-social__list">
          <li class="footer-social__item">
            <a href="/404.php" class="footer-social__link social__vk" target="_blank">
              <svg>
                <use xlink:href="assets/img/icons/vk-icon.svg#vk-icon"></use>
              </svg>
            </a>
          </li>
          <li class="footer-social__item">
            <a href="/404.php" class="footer-social__link social__inst" target="_blank">
              <svg>
                <use xlink:href="assets/img/icons/inst-icon.svg#inst-icon"></use>
              </svg>
            </a>
          </li>
          <li class="footer-social__item">
            <a href="/404.php" class="footer-social__link social__odnoklassniki" target="_blank">
              <svg>
                <use xlink:href="assets/img/icons/odnoklassniki-icon.svg#odnoklassniki-icon"></use>
              </svg>
            </a>
          </li>
        </ul>
      </div>
      <div class="footer-item footer-item__about accordion-tab-js">
        <div class="footer-item__title">О компании</div>
        <ul class="footer-item__menu-list accordion-content-js">
          <li class="footer-item__menu-item">
            <a href="/about.php#mission" class="footer-item__menu-link">Миссия и ценности</a>
          </li>
          <li class="footer-item__menu-item">
            <a href="/about.php#about-team" class="footer-item__menu-link">Команда</a>
          </li>
          <li class="footer-item__menu-item">
            <a href="/about.php#vacancy" class="footer-item__menu-link">Вакансии</a>
          </li>
          <li class="footer-item__menu-item">
            <a href="/about.php" class="footer-item__menu-link">Контакты</a>
          </li>
          <li class="footer-item__menu-item">
            <a href="/franchise.php" class="footer-item__menu-link">Франшиза</a>
          </li>
          <li class="footer-item__menu-item">
            <a href="/representative.php" class="footer-item__menu-link">Представительства</a>
          </li>
        </ul>
      </div>
      <div class="footer-item footer-item__clients accordion-tab-js">
        <div class="footer-item__title">Клиентам и партнёрам</div>
        <ul class="footer-item__menu-list accordion-content-js">
          <li class="footer-item__menu-item">
            <a href="/prices.php" class="footer-item__menu-link">Цены</a>
          </li>
          <li class="footer-item__menu-item">
            <a href="/delete-catalysts.php" class="footer-item__menu-link">Удаление катализатора</a>
          </li>
          <li class="footer-item__menu-item">
            <a href="/prices-wholesalers.php" class="footer-item__menu-link">Оптовикам</a>
          </li>
          <li class="footer-item__menu-item">
            <a href="/loyalty-program.php" class="footer-item__menu-link">Программа лояльности</a>
          </li>
        </ul>
      </div>
      <div class="footer-item footer-item__journal accordion-tab-js">
        <div class="footer-item__title">Журнал</div>
        <ul class="footer-item__menu-list accordion-content-js">
          <li class="footer-item__menu-item">
            <a href="/single-section-journal.php" class="footer-item__menu-link">Каталитические
              системы</a>
          </li>
          <li class="footer-item__menu-item">
            <a href="/single-section-journal.php" class="footer-item__menu-link">Драгоценные металлы</a>
          </li>
          <li class="footer-item__menu-item">
            <a href="/single-section-journal.php" class="footer-item__menu-link">Автомобильная отрасль</a>
          </li>
          <li class="footer-item__menu-item">
            <a href="/single-section-journal.php" class="footer-item__menu-link">Вторичное сырьё</a>
          </li>
        </ul>
      </div>
    </div>
    <div class="footer-copy">© 2020, Sdaykat.ru</div>
  </div>
</footer>

<script src="assets/libs/jquery-libs/jquery.min.js"></script>
<script src="assets/libs/scrollbar/jquery.scrollbar.min.js"></script>
<script src="assets/libs/magnific-popup/jquery.magnific-popup.min.js"></script>
<script src="assets/libs/slick/slick.min.js"></script>
<script src="assets/libs/apexcharts/apexcharts.min.js"></script>

<script src="assets/libs/jquery-mask/jquery.mask.min.js"></script>
<script src="assets/libs/jquery-libs/jquery-ui.min.js"></script>
<script src="assets/libs/jquery-libs/jquery.ui.touch-punch.min.js"></script>

<script src="assets/js/main.js?v=<? echo time(); ?>"></script>